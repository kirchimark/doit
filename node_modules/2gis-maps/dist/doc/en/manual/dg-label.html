<h2 id="labels">Labels</h2><p><dl class="api-incut"><ul class="page-contents"><li><a href="#dglabel">DG.Label</a><ul><li><a href="#example-of usage">Example of usage</a></li><li><a href="#options">Options</a></li></ul></dl></p>
<h3 id="dglabel">DG.Label</h3><p><code>DG.Label</code> class implements a small hint which may appear over a particular object on the map
(above a marker or a vector layer, for example). Also, the tips can be shown not only when you
hover the mouse over the object, but constantly, these tips are called static.</p>
<h4 id="example-of usage">Example of usage</h4><p>To enable the display of tips when you hover over the marker is quite easy:</p>
<pre><code>DG.marker([54.9502, 82.8380], {
    label : &#39;I&#39;m a tip!&#39;
}).addTo(map);
</code></pre><p>For vector layers, you can specify a tip in this way, for example:</p>
...
</code></pre><p>To add a tip to an object that has been already created, you can call the <code>bindLabel</code> method:</p>
...
...
-->
<h4 id="options">Options</h4><p>When you call the <code>bindLabel</code> method, you can set the following options for the tooltip:</p>
<table>
    <thead>
        <tr>
            <th>Option</th>
